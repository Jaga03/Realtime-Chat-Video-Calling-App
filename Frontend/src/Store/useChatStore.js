...
export const useChatStore = create((set, get) => {
  let isPolite = false; // Set based on peer role (e.g., caller is impolite, receiver is polite)
  let makingOffer = false;
  let ignoreOffer = false;

  const updateNegotiationState = (newState) => {
    set({ peerConnection: { ...get().peerConnection, negotiationState: newState } });
  };
  ...
  const handleOffer = async (fromUserId, offer) => {
    ...
    if (!peerConnection || !localStream) {
      ...
      newPeerConnection = new RTCPeerConnection({ iceServers: [{ urls: "stun:stun.l.google.com:19302" }] });
      newLocalStream.getTracks().forEach((track) => newPeerConnection.addTrack(track, newLocalStream));

      newPeerConnection.ontrack = (event) => set({ remoteStreams: event.streams });
      newPeerConnection.onicecandidate = (event) => {
        if (event.candidate && newPeerConnection.localDescription) {
    ...
    const offerCollision = makingOffer || (newPeerConnection.signalingState !== "stable");
    ignoreOffer = !isPolite && offerCollision;
    if (ignoreOffer) {
      console.warn("Ignoring collided offer from:", fromUserId);
      return;
    }

    try {
      console.log("Setting remote description for offer, state:", newPeerConnection.signalingState);
  ...
  return {
    ...
    subscribeToMessages: () => {
      ...
      socket.on("call-user", ({ fromUserId, callType }) => {
        ...
        isPolite = true; // Receiver is polite
        set({ callStatus: "incoming", callType, incomingCall: { fromUserId, callType }, callPartnerId: fromUserId });
      });

      socket.on("offer", async ({ fromUserId, offer }) => handleOffer(fromUserId, offer));

      socket.on("answer", async ({ answer }) => {
        const { peerConnection } = get();
        if (!peerConnection || ignoreOffer) return;

        try {
          console.log("Received answer, setting remote description, state:", peerConnection.signalingState);
    ...
    initiateCall: async (toUserId, callType = "video") => {
      ...
      try {
        ...
        isPolite = false; // Caller is impolite
        makingOffer = true;
